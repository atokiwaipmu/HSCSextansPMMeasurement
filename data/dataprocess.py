...
def plot_giclip(df, df_med, df_sigG, i_min, i_max, img_path):
    ...
    ax0.text(18, 3.5, "Star", size=16,
            horizontalalignment="left",
            verticalalignment="top")

    fig.colorbar(mappable,
                aspect=40, shrink=0.6,
                orientation='horizontal', extend='both',cax=ax2)

    fig.savefig(img_path, bbox_inches="tight")

def replace_inf_nan_dropna(df, col_list):
...
def remove_sextans_C(df):
    df = df[(df.i_sdsscentroid_ra > 151.43) | (df.i_sdsscentroid_ra < 151.33) |
            (df.i_sdsscentroid_dec > 0.13) | (df.i_sdsscentroid_dec < 0.02)].copy()
    return df

def process_dataframe(data_path, img_path, output_path, starflag=False):
    # Read data
    df = pd.read_csv(data_path)
    col_list= ['i_sdsscentroid_ra', 'i_sdsscentroid_dec', "mjd_x", 
                      "HSC_mjd", "i_psfflux_mag", "g_psfflux_mag", 
                      "i_psfflux", "g_psfflux"]
...
def main(data_dir, img_dir, output_dir):
